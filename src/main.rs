...
impl Application for DocxApp {
    ...
    fn view(&self) -> Element<'_, Message> {
        ...
        let content = column(vec![
            ...
            .into(),
            row(vec![
                text("Last Printed:").width(Length::Fixed(120.0)).into(),
                text_input("e.g., 2023-01-01T14:00:00Z", &self.last_printed_date)
...
async fn load_metadata(path: PathBuf) -> Result<(String, String, String), String> {
    ...
    let (created, modified) = {
        ...
        loop {
            match reader.read_event_into(&mut buf) {
                ...
                Ok(Event::Eof) => break,
                Err(e) => return Err(format!("XML parsing error in core.xml: {}", e)),
                _ => (),
            }
            buf.clear();
        }
    ...
    if let Ok(mut app_props_entry) = archive.by_name("docProps/app.xml") {
        let mut app_props_buffer = Vec::new();
        app_props_entry
            .read_to_end(&mut app_props_buffer)
            .map_err(|e| e.to_string())?;
        let mut reader = Reader::from_reader(&app_props_buffer[..]);
        let mut buf = Vec::new();
        loop {
            match reader.read_event_into(&mut buf) {
                Ok(Event::Start(ref e)) if e.name().as_ref() == b"LastPrinted" => {
                    last_printed = reader.read_text(e.name()).unwrap_or_default().to_string();
                }
                Ok(Event::Eof) => break,
                Err(e) => return Err(format!("XML parsing error in app.xml: {}", e)),
                _ => (),
            }
            buf.clear();
        }
...
async fn save_metadata(
    ...
    if !last_printed_date.is_empty() {
        DateTime::parse_from_rfc3339(&last_printed_date.replace("Z", "+00:00")).map_err(|_| {
            "Invalid 'Last Printed' format. Use ISO 8601 (e.g., YYYY-MM-DDTHH:MM:SSZ)."
                .to_string()
        })?;
    }
...
fn generate_core_xml(
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Err(e) => return Err(format!("XML (core) processing error: {}", e)),
        }
        buf.clear();
    }
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::End(e)) => {
                ...
                if e.name().as_ref() == b"Properties" && !found_last_printed && !new_last_printed.is_empty() {
                    writer.create_element("LastPrinted").write_text_content(BytesText::new(new_last_printed)).unwrap();
                }
            ...
            Err(e) => return Err(format!("XML (app) processing error: {}", e)),
        }
        buf.clear();
    }
